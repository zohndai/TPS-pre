...
def tile(x, count, dim=0):
    ...
    """
    perm = list(range(len(x.size())))#[0, 1, 2]#src:[0,1,2]
    #print("tile perm:{}".format(perm))
    if dim != 0:
        perm[0], perm[dim] = perm[dim], perm[0] # [1, 0, 2]  #[1, 0, 2]
        x = x.permute(perm).contiguous() # 转秩 scr: [batch_size, seq_len, 1]
    out_size = list(x.size())# [batch_size, seq_len, d_m]

    out_size[0] *= count #[64 * 5, seq_len, d_m]
    #print("2 out_size is :{}".format(out_size))
    batch = x.size(0) #64
    x = x.view(batch, -1) \
         .transpose(0, 1) \
         .repeat(count, 1) \
         ...
         .contiguous() \
         .view(*out_size)
    #print("x' tensor_size:{}".format(x.size()))
    #print("x' tensor_:{}".format(x))
    if dim != 0:
        x = x.permute(perm).contiguous() #转秩
    return x
