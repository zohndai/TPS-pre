...
class ArgumentParser(cfargparse.ArgumentParser):
    ...
    def defaults(cls, *args):
        ...
        dummy_parser = cls()#parser = ArgumentParser()
        for callback in args:
            callback(dummy_parser)#opts.model_opt(paser)添加模型参数
        defaults = dummy_parser.parse_known_args([])[0]#parse_known_args() -> (args, argv),解析出默认参数
    ...
    def ckpt_model_opts(cls, ckpt_opt):
        ...
        opt = cls.defaults(opts.model_opts) #解析出的所有模型都有的默认参数
        opt.__dict__.update(ckpt_opt.__dict__)#对默认namepace的所有属性(即默认参数)预训练模型更新
        return opt
