...
def build_torch_optimizer(model, opt):
    ...
    if opt.model_dtype == 'fp16':#False
        import apex
        if opt.optim != 'fusedadam':
            # In this case use the new AMP API from apex
            loss_scale = "dynamic" if opt.loss_scale == 0 else opt.loss_scale#True
            model, optimizer = apex.amp.initialize(
                [model, model.generator],
                optimizer,
                opt_level=opt.apex_opt_level,#O1:混合精度
...
class Optimizer(object):
    ...
    @classmethod
    def from_opt(cls, model, opt, checkpoint=None):#(my_model, opt_T, checkpoint=checkpoint_train)
        ...
        if opt.train_from and checkpoint is not None:
            ...
            ckpt_state_dict = {}
            if isinstance(optim, Optimizer):  # Backward compatibility.#False
        ...
        if opt.model_dtype == "fp16":#False
            if opt.optim == "fusedadam":
                ...
                optimizer._fp16 = "amp"
        if optim_state_dict:
            #print("=============optim_state_dict{}---------".format(optim_state_dict.keys()))
    ...
    def step(self):
        ...
        if self._fp16 == "legacy":#self._fp16=None
        ...
        for group in self._optimizer.param_groups:
            ...
            if self._fp16 is None and self._max_grad_norm > 0:#False,max_grad_norm=0
