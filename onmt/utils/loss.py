...
class LossComputeBase(nn.Module):
    ...
    def __call__(self,
        ...
        if trunc_size is None:#False trunc_size = seq_len
            trunc_size = batch.tgt.size(0) - trunc_start
        trunc_range = (trunc_start, trunc_start + trunc_size)
        shard_state = self._make_shard_state(batch, output, trunc_range, attns)

        if shard_size == 0:# training:False, validation:True
    ...
    def _stats(self, loss, scores, target):
        ...
        #print("~~~~~~target{}{}".format(target, target.size()))

        #print("######scores{}{}".format(scores, scores.size()))
...
class NMTLossCompute(LossComputeBase): #(criterion, loss_gen, lambda_coverage=opt.lambda_coverage=0,
                                       # lambda_align=opt.lambda_align=0)
    """
    ...
    def _make_shard_state(self, batch, output, range_, attns=None):
        ...
        if self.lambda_coverage != 0.0:#False
        ...
        if self.lambda_align != 0.0:#False
        ...
        #print("shard_state_keys:{}".format(shard_state.keys()))
        return shard_state
    ...
    def _compute_loss(self, batch, output, target, std_attn=None,
        ...
        #print("---bottled_output--{}-{}".format(bottled_output, bottled_output.size()))

        scores = self.generator(bottled_output)
        #print("===scores====={}-{}".format(scores, scores.size()))

        gtruth = target.view(-1)
        #print("####gtruth####{}-{}".format(gtruth, gtruth.size()))

        #print("@@@@@@self.criterion.reduction--{}".format(self.criterion.reduction))
...
def filter_shard_state(state, shard_size=None):
    #print("filter_shard_state state:{}".format(state)) state->dict{"output": Tensors, "Target": Tensors}
    for k, v in state.items():
        ...
        if v is not None:
            ...
            if isinstance(v, torch.Tensor):
                #print("!!!!!!!!!v{} v.size{}".format(v, v.size()))
                for v_chunk in torch.split(v, shard_size):
                    #print("@@@@@@@@@@v_chunk{} v_chunk.size{}".format(v_chunk, v_chunk.size()))
...
def shards(state, shard_size, eval_only=False):
    ...
    if eval_only:
        ...
        # are not None.
        non_none = dict(filter_shard_state(state, shard_size))#{"output":(Tensor, [chunk_tensors]), "target":(tensor,[])}
        ...
        # with the keys.
        for shard_tensors in zip(*values):#((T1, t1), (T2, t2), (T3, t3)...)
            #print("dict---------{}!!!!!!!".format(list(zip(*values))))
            #print("11111111111{}22222".format(dict(zip(keys, shard_tensors))))
            yield dict(zip(keys, shard_tensors))#dict((output, T1), (target, t1))...
