...
class TextDataReader(DataReaderBase):
    def read(self, sequences, side, _dir=None):
        ...
        Yields:
            ...
            fields.
        """
        #print("this dataset reader....")
        assert _dir is None or _dir == "", \
            "Cannot use _dir with TextDataReader."
        if isinstance(sequences, str):#false
            print("sata is a string2")
            sequences = DataReaderBase._read_file(sequences)
        for i, seq in enumerate(sequences):
            if isinstance(seq, six.binary_type):#True
                #print("yes, this is six.binary_type")
                seq = seq.decode("utf-8")
            yield {side: seq, "indices": i}
            #{'src': '[O-] N = O . [OH]', 'indices': 0}
            #{'src': '[O] C C C C . O = [O+] [O-]', 'indices': 1}
            #...
...
class TextMultiField(RawField):
    ...
    def process(self, batch, device=None):
        ...
        if self.base_field.include_lengths:
            ...
            #print("base_data:{}".format(base_data))
#base_data[[seq_len, batch_size]]
        feats = [ff.process(batch_by_feat[i], device=device)
                 for i, (_, ff) in enumerate(self.fields[1:], 1)]
        #print("feats:{}".format(feats))
        levels = [base_data] + feats
        #levels:[tensor([[seq_len, batch_size]])]
        #print("levels:{}".format(levels))
        # data: seq_len x batch_size x len(self.fields)
        data = torch.stack(levels, 2)
        #先扩维，再堆叠
        #print("data_stacked:{}".format(data))
        if self.base_field.include_lengths:
            return data, lengths
        else:
            return data
