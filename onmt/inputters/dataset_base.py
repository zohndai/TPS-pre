...
class Dataset(TorchtextDataset):
    ...
    def __init__(self, fields, readers, data, dirs, sort_key,
        ...
        can_copy = 'src_map' in fields and 'alignment' in fields#False
        
        #print("readers:{}".format(readers))
        #print("readers -> : {}".format(readers))
        read_iters = [r.read(dat[1], dat[0], dir_) for r, dat, dir_
                      in zip(readers, data, dirs)]
        #read_iters = [generators...]

        #[{'src': '[O-] N = O . [OH]', 'indices': 0}, {'src': '[O] C C C C . O = [O+] [O-]', 'indices': 1}...]
        #print("=========readers: {}".format(next(r.read(dat[1], dat[0], dir_) for r, dat, dir_ in zip(readers, data, dirs))))

        # self.src_vocabs is used in collapse_copy_scores and Translator.py
        self.src_vocabs = []
        examples = []
        #for i in zip(*read_iters):
            #print(i)
        #print("type_reader_iters:{}".format(type(read_iters)))
        for ex_dict in starmap(_join_dicts, zip(*read_iters)):
            #print("ex_dict:{}".format(ex_dict))
            if can_copy:#False
            ...
            #fields: {'src': MultiTextField, 'tgt': MultiTextField, 'indices': field.Field}
            
            #ex_dict: {'src': '[O-] N = O . [OH]\r\n', 'indices': 0}...
            
            #ex_fields: {'src': [("src",MultiTextField)], 'indices': [("indices", field.Field)]}

            ex_fields = {k: [(k, v)] for k, v in fields.items() if
                         k in ex_dict}
            #print("ex_fields['src'type]: {}".format(ex_fields['src']))
        ...
        for _, nf_list in ex_fields.items():
            #print("nf_list:{}".format(nf_list))
            assert len(nf_list) == 1
            fields.append(nf_list[0])
        #fields: [('src',TextMultiField), ('indices', field.Field )]
        super(Dataset, self).__init__(examples, fields, filter_pred)
        

        #print("self.src_vocab: {}".format(self.src_vocabs))
        #self.src_vocabs = []
