...
class OrderedIterator(torchtext.data.Iterator):
    ...
    def __iter__(self):
        ...
        """
        while True:
            self.init_epoch()# out:self.batchs -> generator object
            for idx, minibatch in enumerate(self.batches):#minibatch:[(id,[examples...])...]
                # fast-forward if loaded from state
                if self._iterations_this_epoch > idx:
                    continue
                self.iterations += 1
                self._iterations_this_epoch += 1
                if self.sort_within_batch:   #True
                    ...
                    if self.sort: #sort=False
                        minibatch.reverse()
                    else:
                        #print('sort=False')
                        minibatch.sort(key=self.sort_key, reverse=True)
                if self.yield_raw_example: #yield_raw_example=False
                    yield minibatch[0]
                else:
                    #print("yield=False")
                    yield torchtext.data.Batch(
                        ...
                        self.dataset,
                        self.device)
            if not self.repeat: #repeat=False
                #print("no repeat")
                return
...
class DatasetLazyIter(object):
    ...
    def _iter_dataset(self, path):
        ...
        for batch in cur_iter:
            self.dataset = cur_iter.dataset#self.dataset=cur_dataset
            yield batch
    ...
    def __iter__(self):
        num_batches = 0
        paths = self._paths
        if self.is_train and self.repeat:#True
        ...
        if self.is_train and not self.repeat and \
           num_batches % self.num_batches_multiple != 0:#False
