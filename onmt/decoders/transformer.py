...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        _attns_list = list(attns[0])

        # print(f"cross attn list length {len(_attns_list)}")
        ...
        # print("attns.size:{}".format(attns[0].size()))
        ...
        return output, top_attn, attn_array #=None
    ...
    def _forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
        ...
        mid, attns = self.context_attn(memory_bank, memory_bank, query_norm,
                                       ...
                                       attn_type="context")
        # print(f"contex attn size: {attns.size()}")
...
class TransformerDecoder(DecoderBase):
    ...
    def forward(self, tgt, memory_bank, step=None, **kwargs):# (dec_in, memory_bank,
        ...
        # print(f"_cross-attn dimension:{attn_array.ndim}")

        if self.output and attn_array.ndim==2:
            attn_file = self.output.replace(".txt", "_cross-attn.csv")
            pd.DataFrame(attn_array).to_csv(attn_file, mode="a", header=None, index=False)
