...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,
        ...
        if feat_merge == 'sum':#False
            feat_dims = [word_vec_size] * len(feat_vocab_sizes)
        elif feat_vec_size > 0:#False
            feat_dims = [feat_vec_size] * len(feat_vocab_sizes)
        else:#->[]
            feat_dims = [int(vocab ** feat_vec_exponent)
                         for vocab in feat_vocab_sizes]
            #print(f"feat_dims in else:{feat_dims}")
        vocab_sizes.extend(feat_vocab_sizes)#[74]不变
        emb_dims.extend(feat_dims)#[384]不变
        pad_indices.extend(feat_padding_idx)#[1]不变
        #print(vocab_sizes)
        #print(emb_dims)
        #print(pad_indices)
        ...
        emb_params = zip(vocab_sizes, emb_dims, pad_indices)#->[(74, 384, 1)]
        ...
        self.embedding_size = (sum(emb_dims) if feat_merge == 'concat'
                               else word_vec_size)#->emb_dims=384
        ...
        if feat_merge == 'mlp' and len(feat_vocab_sizes) > 0:#False
            in_dim = sum(emb_dims)
